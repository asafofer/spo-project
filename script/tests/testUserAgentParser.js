...
const SUPPORTED_BROWSERS = new Set([
    'facebook', 'instagram', 'gsa', 'samsung browser', 'miui browser', 'whale', 'silk', 
    'ucbrowser', 'yandex', 'avast secure browser', 'avg secure browser', 'opera', 'edge', 
    'firefox', 'chrome webview', 'chrome', 'mobile safari', 'safari', 'android browser', 'webkit'
]);

const SUPPORTED_OS = new Set([
    'android', 'ios', 'windows', 'mac os', 'chromium os', 'ubuntu', 'linux'
]);
...
function runTests(dataset, type) {
    ...
    dataset.forEach((testCase) => {
        ...
        let expectedRaw = testCase.expect.name;
        ...
        const supportedSet = type === 'Browsers' ? SUPPORTED_BROWSERS : SUPPORTED_OS;
        ...
        const actual = type === 'Browsers' ? result.browser : result.operatingSystem;
